...
TO DO : 
 ...
 -   if this is the first boot select a default FM frequency and a default volume level.
...
void setup() {
  ...
  if (frequence == 255) // if frequence is null
  {
    frequence = 10250; // fix a frequency to start
  }

  trigger3(); // read the saved volume inside the arduino eeprom
  delay(250); // wait 1/4 seconds
  if (volume == 255) // if volume is null
  {
    volume = 5; // fix a volume level to start
  }
