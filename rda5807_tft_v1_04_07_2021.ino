...
void showFrequency()
{
  ...
  tft.print('.');
  char sfreq[40];
  sprintf(sfreq,"\nVous êtes à l'écoute de %u MHz ",rx.getFrequency());
...
void showRSSI()
{
  ...
  char level[40];
  //sprintf(aux,"\nVous êtes à l'écoute de %u MHz | RSSI: %3.3u dbUv | Vol: %2.2u | Stereo: %s ",rx.getFrequency(), rx.getRssi(), rx.getVolume(), (rx.isStereo()) ? "Oui" : "Non" );
  sprintf(level,"\nLe niveau de réception est de %i dBuV ",rx.getRssi());
...
void showStereoMono() {
  ...
  char mode[40];
  //sprintf(aux,"\nVous êtes à l'écoute de %u MHz | RSSI: %3.3u dbUv | Vol: %2.2u | Stereo: %s ",rx.getFrequency(), rx.getRssi(), rx.getVolume(), (rx.isStereo()) ? "Oui" : "Non" );
  sprintf(mode,"\nLe mode est %s",(rx.isStereo()) ? "Stéréo" : "Mono" );
  Serial.println(mode);
...
char *stationName;
char *rdsTime;
char bufferStatioName[30];
char bufferRdsMsg[80];
...
void showRDSMsg()
{
  ...
  rdsMsg[80] = bufferRdsMsg[80] = '\0';   // Truncate the message to fit on display.  You can try scrolling
  if (strcmp(bufferRdsMsg, rdsMsg) == 0)
    return;
  printValue(5, 90, bufferRdsMsg, rdsMsg, 7, COLOR_YELLOW);
  char moderds[80];
  sprintf(moderds,(bufferRdsMsg, rdsMsg));
  Serial.println(moderds);
  delay(250);
}
...
void showRDSStation()
{
  ...
  char rdsstation[80];
  sprintf(rdsstation,(bufferStatioName, stationName));
  Serial.println(rdsstation);
}
...
void clearRds() {
  ...
  bShow = false;
  //char rdsclear[0];
  //rdsMsg[0] = bufferRdsMsg[0];
  //sprintf(rdsclear,(bufferStatioName, stationName));
  //Serial.println(rdsclear);
}
...
void showRds() {
  ...
  //char moderds[20];
  //sprintf(moderds, "\nLe RDS est %s",(bRds) ? "actif" : "inactif" );
  //Serial.println(moderds);
  checkRDS();
...
void setup()
{
  ...
  rx.setFrequency(10140); // It is the frequency you want to select in MHz multiplied by 100.
